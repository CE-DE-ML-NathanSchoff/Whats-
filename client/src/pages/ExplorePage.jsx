...
function getStage(watersCount) {
  if (watersCount >= 10) return 'oak'
  if (watersCount >= 6) return 'tree'
  if (watersCount >= 3) return 'sapling'
  if (watersCount >= 1) return 'sprout'
  return 'seed'
}
...
export default function ExplorePage() {
  ...
  const [explorePosts, setExplorePosts] = useState(EXPLORE_POSTS)
  ...
  useEffect(() => {
    async function loadEvents() {
      try {
        ...
        if (comms.length > 0) {
          const eventsList = await communitiesApi.getEvents(comms[0].id);
          const posts = eventsList.map((e) => ({
            id: e.id,
            title: e.title,
            content: e.description || '',
            lat: 0, lng: 0,
            waters_count: e.waters_count ?? e.rsvp_count ?? 0,
            growth_stage: getStage(e.waters_count ?? 0),
            is_branch: false,
            branch_count: 0,
            time_label: e.event_time || e.event_date || '',
          }));
  ...
  // ── Filtering + sorting ─────────────────────────────────────────────────────
  const visiblePosts = explorePosts
    .filter((p) =>
      searchQuery === '' ||
