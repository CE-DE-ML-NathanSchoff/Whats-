...
function eventToPost(event) {
  const waters = event.waters_count ?? 0;
  let stage = 'seed';
  if (waters > 10) stage = 'oak';
  else if (waters > 5) stage = 'sapling';
  else if (waters > 2) stage = 'sprout';
  else if (waters > 0) stage = 'seed';

  return {
    id: event.id,
    title: event.title,
    content: event.description || '',
    lat: 39.9526 + (Math.random() - 0.5) * 0.04,
    lng: -75.1652 + (Math.random() - 0.5) * 0.04,
    waters_count: waters,
    growth_stage: stage,
    is_branch: false,
  };
...
export default function MapPage() {
  ...
  useEffect(() => {
    async function loadEvents() {
      try {
        const comms = await communitiesApi.list({ type: 'LOCATION' });
        const list = comms.communities ?? comms ?? [];
        if (list.length === 0) return;
        const eventsData = await communitiesApi.getEvents(list[0].id);
        const items = eventsData.events ?? eventsData ?? [];
        if (items.length > 0) {
          setPosts(items.map(eventToPost));
        }
      } catch {
        // keep SEED_POSTS as fallback
      }
    }
    loadEvents();
  }, [])
  ...
  useSocket({
    'event:created': (event) => {
      setPosts((prev) => [...prev, eventToPost(event)]);
    },
    'event:updated': (event) => {
      setPosts((prev) => prev.map((p) => p.id === event.id ? { ...p, ...eventToPost(event) } : p));
    },
    'event:watered': ({ eventId }) => {
      setPosts((prev) => prev.map((p) => {
        if (p.id !== eventId) return p;
        const newCount = (p.waters_count ?? 0) + 1;
        const stage = newCount > 10 ? 'oak' : newCount > 5 ? 'sapling' : newCount > 2 ? 'sprout' : 'seed';
        return { ...p, waters_count: newCount, growth_stage: stage };
      }));
    },
  });
